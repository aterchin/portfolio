...
<Aside variant="info" title="Why escapeHtml?">
When highlight.js does not recognize a language, the fallback path renders plain text through `dangerouslySetInnerHTML`. Without escaping, characters like `&lt;` or `&amp;` in the source would be parsed as HTML. `escapeHtml` keeps unknown-language blocks safe and displays the raw characters you typed.
</Aside>
